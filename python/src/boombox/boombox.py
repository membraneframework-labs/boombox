...
class Boombox(process.Process):
    ...
    def _download_erlang_boombox(self) -> None:
        ...
        try:
            self._version = importlib.metadata.version("boomboxlib")
        except importlib.metadata.PackageNotFoundError:
            self._version = "dev"

        self._data_dir = platformdirs.user_data_dir(
            appname="boombox", ensure_exists=True, version=self._version
        )

        if os.path.exists(os.path.join(self._data_dir, "bin", "server")):
            print("Erlang release already present.")
            return
        print("Erlang release not found, downloading...")

        releases_url = "https://github.com/membraneframework-labs/boombox/releases"
        ...
        if system == "linux" and arch == "x86_64":
            ...
            release_tarball = "boombox-server-macosx-arm.tar.gz"
        else:
